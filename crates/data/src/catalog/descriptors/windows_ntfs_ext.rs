...
records what they became).",
    ...
    sources: &[
        ...
        "https://github.com/tuxera/ntfs-3g/blob/edge/include/ntfs-3g/layout.h",
        // Source: https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/robocopy (/COPY flag O = owner information: a copy can carry the source owner SID)
    ...
    evidence_caveats: &[
        ...
        "An owner SID is not authorship: it records the security context that created the file or later took ownership of it, an administrator can change it, and copy tools that preserve owner information (robocopy /COPY:O) carry it from another volume, so files owned by different SIDs show multiple user contexts that need corroboration, not multiple people",
    ],
    volatility: Some(crate::volatility::VolatilityClass::Residual),
