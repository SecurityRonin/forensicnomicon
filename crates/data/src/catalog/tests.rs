...
const EXPECTED_CATALOG_LEN: usize = 6754;
...
mod tests_browser_session_memory {
    ...
    fn browser_chrome_session_memory_has_no_file_path() {
        ...
        assert!(
            d.file_path.is_none(),
            "memory artifact must not have a file_path"
        );
    }
    ...
    fn browser_chrome_session_memory_cites_memf_source() {
        ...
        assert!(
            d.sources
                .iter()
                .any(|s| s.contains("memory-forensic") || s.contains("memf")),
    ...
    fn browser_chrome_session_related_includes_memory() {
        ...
        assert!(
            d.related_artifacts
                .contains(&"browser_chrome_session_memory"),
    ...
    fn browser_chrome_session_caveat_mentions_memory() {
        ...
        assert!(
            has_memory_caveat,
            "on-disk session must caveat that memory counterpart is richer"
        );
    }
}
