...
**Status:** proposal / not yet executed. **Date:** 2026-06-28.
...
- Cut `1.0` now, or stay `0.11.x` and only normalize pins? (1.0 recommended.)
- For B: confirm the structural-vs-detection classification of each table, and
  whether to keep a `forensicnomicon` facade crate long-term or hard-cut.
- Crate names: `forensicnomicon-core` + `forensicnomicon-data`, or
  `forensicnomicon` (engine) + `forensicnomicon-catalog` (data)?
